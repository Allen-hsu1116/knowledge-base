...
RAG 可緩解 LLM 的三類限制：**知識截止日**、**幻覺問題**與**專業知識缺失**，讓模型利用可更新的外部來源回答。**[⚠️ 可能過時] 原文「解決了三大先天限制」過度概括**：檢索仍可能失敗，生成也可能誤讀來源；有引用不等於答案正確。Anthropic 的 Contextual Retrieval 實驗仍報告非零檢索失敗率，不能把檢索改善視為消除幻覺的保證（見來源）。
...
- [Anthropic：Contextual Retrieval](https://www.anthropic.com/engineering/contextual-retrieval) — 檢索失敗率與重排序實驗；不是最終回答零幻覺保證。
