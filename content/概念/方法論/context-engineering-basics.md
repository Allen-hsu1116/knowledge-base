...
語言模型不喜歡壓縮——給模型 erase 工具它拒絕使用，必須強制執行或 RL 訓練。Subagent 是天然的壓縮方式：return 後繁瑣過程從 context 消失，長度呈鋸齒狀升降。Agentic CE 則把 Context Engineering 也交給 LLM，用 Dynamic Cheatsheet、Playbook 或 Recursive Language Model 實現。 知識庫方法論（raw → wiki → lint）本身就是一種 Context Engineering：raw/ 是外部記憶（M），wiki/ 是精煉後進入 LLM 的 context（P），lint 就是 ACON 的角色——檢查壓縮是否丟失重要資訊。
