...
- **In-page GUI Agent** — 僅依賴頁面 DOM 操作的輕量 GUI Agent，不需多模態 LLM（VLM）。透過自然語言指令直接操作網頁 DOM 元素（點擊按鈕、填寫表單、導航頁面），省去截圖和視覺辨識的成本。與全桌面 Computer-Use Agent 形成對比：後者需要 VLM 理解螢幕畫面並模擬鍵盤滑鼠，適合任意桌面應用；In-page Agent 限於網頁但更精準、更快、成本更低，適合大規模網頁自動化。
