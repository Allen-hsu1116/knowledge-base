...
專案把 `prompt → context → harness → loop → graph` 視為五個不同的控制問題，而不是幾個可以互換的名詞。這不是章節順序；實際學習順序是 Stage 2 Prompt → Stage 3 Agent Loop → Stage 4 Workflow Graph／Framework → Stage 5 工具與規則 → Stage 6 Context Engineering → Stage 7 production。Prompt 說清楚一次任務；Context 決定模型此刻看到什麼；Harness 提供規則、工具與環境；Loop 控制觀察—行動—修正；Graph 則讓多階段流程具備明確結構。
...
2. Track A 按 `A1 → A2 → Stage 5 → A3 → Stage 8` 前進；Stage 5 核心是 5.1–5.4，5.5–5.8 按需選讀。
3. 一次只走一個 Stage，先跑官方範例與不連網測試。
4. 每次只改一件事，改完立即重跑驗證。
5. 達成章節完成條件後再前進，不把「看過」當成「會做」。
6. 完成 A3 或 Stage 7 後，以 Capstone 整合成可評測、可復原的 Agent。
...
| [Datawhale Hello-Agents](https://github.com/datawhalechina/hello-agents) | 完整章節與深度實作 | 本專案 README 推薦的深入教材 |
| [liyupi/ai-guide](https://github.com/liyupi/ai-guide) | 廣度資源庫 | 與本專案的循序學習安排互補 |
