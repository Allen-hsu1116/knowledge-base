...
核心洞察是「環境工程」（Environment Engineering）：環境定義系統。與其給 Agent 一堆分散的工具和 context，不如給它一個完整的、連貫的工作環境 — 記憶不重置、目標不丟失、狀態不斷裂。
...
- **Agent Harness**：運行時的穩定邊界，定義 Agent 能做什麼、不能做什麼，保證操作可檢查、可回滾
