...
這是三省六部的殺手鐧。CrewAI 和 AutoGen 的 Agent 做完就交，沒人檢查品質。三省六部的門下省專職做三件事：

1. **審查方案品質** — 中書省的規劃是否完備？子任務拆解是否合理？
2. **封駁不合格產出** — 不是 warning，是直接打回重做
3. **強制返工循環** — 直到方案達標才放行，沒有例外

每一個旨意都必須經過門下省，沒有 bypass。1300 年前唐太宗就想明白了：不受制約的權力必然會出錯。

### 十二部制 Agent 架構

```
皇上（你）→ 太子（分揀）→ 中書省（規劃）→ 門下省（審議）→ 尚書省（派發）→ 六部（執行）→ 回奏
```

- **太子** — 消息分揀，閒聊自動回覆，旨意才建任務
- **三省**（中書·門下·尚書）— 規劃、審議、派發
- **七部**（戶·禮·兵·刑·工·吏 + 早朝官）— 專項執行
- 嚴格的權限矩陣 — 誰能給誰發訊息，白紙黑字
- 狀態流轉校驗 — `kanban_update.py` 強制合法轉換路徑，非法狀態跳轸被拒絕
- 每個 Agent 獨立 Workspace、獨立 Skills、獨立模型

### 軍機處看板（10 個功能面板）

| 面板 | 功能 |
|------|------|
| 📋 旨意看板 | 按狀態列展示任務、省部篩選、心跳徽章、叫停/取消/恢復 |
| 🔭 省部調度 | 任務數量視覺化、部門分布、Agent 健康狀態 |
| 📜 奏折閣 | 已完成旨意歸檔、五階段時間線、一鍵複製 Markdown |
| 📜 旨庫 | 9 個預設聖旨模板、參數表單、一鍵下旨 |
| 👥 官員總覽 | Token 消耗排行、活躍度、會話統計 |
| 📰 天下要聞 | 每日自動採集科技/財經資訊 + 飛書推送 |
| ⚙️ 模型配置 | 每 Agent 獨立切換 LLM，5 秒生效 |
| 🛠️ 技能配置 | 各省部已安裝 Skills 一覽 |
