...
- ⚡ **Token Optimization** → [[Token-Optimization]]（caveman 是 token 壓縮的實戰方案）
- 📝 **Prompt Engineering** → [[Prompt-Engineering]]（穴居人語是一種 prompt engineering 技巧）

## 簡介

Caveman 是一個 Claude Code / Codex plugin，核心理念很簡單：**為什麼用很多字，當少少字就夠了？**
...
- AI 用穴居人語回答，省掉 75% output token
- 技術準確度 100% 保留
- 可切換四種強度等級

### Input 壓縮（46% 節省）

- caveman-shrink MCP middleware 自動壓縮送進 LLM 的 input
- 攔截命令輸出，智慧過濾冗餘資訊
- 類似 [[rtk]] 但用 MCP 介面而非 CLI proxy
...
- **TypeScript**：核心 plugin
- **MCP**：caveman-shrink middleware
- **支援**：Claude Code、Codex CLI、Cursor、OpenCode 等 30+ harness

## 相關概念

- [[Token-Optimization]] — Token 壓縮的整體技術
- [[Prompt-Engineering]] — 穴居人語是 prompt engineering 的實戰應用
- [[rtk]] — 另一種 token 壓縮方案（CLI proxy）
